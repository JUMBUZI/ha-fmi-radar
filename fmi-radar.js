...
class FmiRadar extends HTMLElement {
        ...
        if (!this.content) {
            this.innerHTML = `
                <ha-card style="overflow:clip;">
                    <div class="card-content" style="padding: 0;"></div>
                </ha-card>
            `
            ...
            if (String(this.config.station_id).includes('suomi')) {
                if (this.config.station_id === 'suomi1h') {
                    this.jsonUrls.push(
                        `https://${ this.host }/apps/list-finland-radar-images/index.php?product-id=${ this.config.station_id }&isforecast=false`
                    )
                    this.jsonUrls.push(
                        `https://${ this.host }/apps/list-finland-radar-images/index.php?product-id=${ this.config.station_id }&isforecast=true`
                    )
                } else {
                    this.jsonUrls.push(
                        `https://${ this.host }/apps/list-finland-radar-images/index.php?product-id=${ this.config.station_id }&flash=true`
                    )
                }
            } else {
                this.jsonUrls.push(
                    `https://${ this.host }/apps/list-local-weather-radar-images/index.php?lang=fi&station=${ this.config.station_id }&timezone=UTC`
                )
            }
    ...
    getImages = (i, len) => {
        if (i === 0) { this.radarImages = [] }

        if (i >= len) {
            this.renderCard()
            if (!this.slideshow_id) { this.startSlideshow() }
            this.image_refresh_id = setTimeout(
                this.getImages,
                this.config.image_refresh_interval * 1000,
                0,
                this.jsonUrls.length
            )
            return
        }
        ...
        req.onreadystatechange = () => {
            if(req.readyState === XMLHttpRequest.DONE && req.status === 200) {
                try {
                    ...
                    this.radarImages = [
                        ...this.radarImages,
                        ...jsonData.images.map(img => ({ ...img, url: img.url.replace('cdn.fmi.fi', this.host) }))
                    ]
    ...
    renderCard = () => {
        ...
        this.radarImages.forEach((image) => {
            ...
            radarTime.style =
                'position: absolute; '
                + this.translation[this.config.time_vertical_pos] + ': 16px; '
                + this.translation[this.config.time_horizontal_pos] + ': 16px; '
                + 'font-weight: ' + this.config.time_font_weight + '; '
                + 'font-size: ' + this.config.time_font_size + 'em; '
                + 'color: ' + this.config.time_font_color + ';'
            let radarTimeDate = new Date(image.epoch)
            let radarTimeStr
            if (!this.config.time_prefix) {
                radarTimeStr =
                    radarTimeDate.getDate()
                    + '.' + (radarTimeDate.getMonth()+1)
                    + '. ' + ('0' + radarTimeDate.getHours()).slice(-2)
                    + ':' + ('0' + radarTimeDate.getMinutes()).slice(-2)
            } else {
                radarTimeStr =
                    radarTimeDate.getDate()
                    + '.' + (radarTimeDate.getMonth()+1)
                    + '. ' + this.config.time_prefix
                    + ' ' + ('0' + radarTimeDate.getHours()).slice(-2)
                    + ':' + ('0' + radarTimeDate.getMinutes()).slice(-2)
            }
    ...
    setConfig(config) {
        if (!config.station_id) {
            throw new Error(
                'Määritä "station_id" konfiguraattorissa (Ilmatieteen laitoksen kohteen ID)'
            )
        }
        if (!config.speed_ms) {
            throw new Error(
                'Määritä "speed_ms" konfiguraattorissa (tutkakuvien siirtymäaika millisekunteina)'
            )
        }
        if (!config.time_horizontal_pos || (config.time_horizontal_pos !== 'vasen' && config.time_horizontal_pos !== 'oikea')) {
            throw new Error(
                'Määritä "time_horizontal_pos" konfiguraattorissa (tutkakuvan päälle tulevan päivämäärän ja kellonajan vaakasuuntainen sijainti, vasen tai oikea)'
            )
        }
        if (!config.time_vertical_pos || (config.time_vertical_pos !== 'ylä' && config.time_vertical_pos !== 'ala')) {
            throw new Error(
                'Määritä "time_vertical_pos" konfiguraattorissa (tutkakuvan päälle tulevan päivämäärän ja kellonajan pystysuuntainen sijainti, ylä tai ala)'
            )
        }
        if (!config.time_font_weight) {
            throw new Error(
                'Määritä "time_font_weight" konfiguraattorissa (tutkakuvan päälle tulevan päivämäärän ja kellonajan fontin paksuus)'
            )
        }
        if (!config.time_font_size) {
            throw new Error(
                'Määritä "time_font_size" konfiguraattorissa (tutkakuvan päälle tulevan päivämäärän ja kellonajan fontin koko em:nä)'
            )
        }
        if (!config.time_font_color) {
            throw new Error(
                'Määritä "time_font_color" konfiguraattorissa (tutkakuvan päälle tulevan päivämäärän ja kellonajan fontin väri)'
            )
        }
        if (!config.image_refresh_interval) {
            throw new Error(
                'Määritä "image_refresh_interval" konfiguraattorissa (kuinka monen sekunnin välein ladataan uudet kuvat)'
            )
        }
        this.config = config
    ...
    static getStubConfig() {
        return {
            station_id: 100971,
            speed_ms: 500,
            time_horizontal_pos: 'vasen',
            time_vertical_pos: 'ylä',
    ...
    render() {
        ...
        return html`
            <div class="card-config">
                <div>
                    <p>
                        Voit käyttää Ilmatieteen laitoksen kohteen ID:n tilalla myös arvoja
                        <b>suomi1h</b>, <b>suomi</b>, <b>etela-suomi</b>, <b>keski-suomi</b>
                        tai <b>pohjois-suomi</b> saadaksesi laajemman tutkakuvan.
                    </p>
                    <ha-input
